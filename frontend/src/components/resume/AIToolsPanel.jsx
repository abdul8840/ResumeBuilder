                ...
                <ol className="space-y-2">
                  {customization.priorityActions.map((action, i) => (
                    <li key={i} className="flex items-start gap-2.5 p-2.5 bg-indigo-50 rounded-xl border border-indigo-100">
                      <span className="w-5 h-5 rounded-full gradient-bg text-white text-xs flex items-center justify-center font-bold flex-shrink-0 mt-0.5">
                        {i + 1}
                      </span>
...
const AIToolsPanel = ({ resumeId, onClose }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      {/* Tool tabs */}
      <div className="grid grid-cols-4 gap-1 p-3 border-b border-slate-100 flex-shrink-0">
